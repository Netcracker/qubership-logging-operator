...
- **Build** — run the documented local build or the same Maven/Go target CI uses (e.g. workflow `maven-command`, `make test-unit`, image build in bootstrap).
- **Smoke** — use the shallowest documented path that runs the changed component and emits stdout or pod logs (local deploy scripts, `dev/` workflows, documented startup, or a CI job that exercises the service).
