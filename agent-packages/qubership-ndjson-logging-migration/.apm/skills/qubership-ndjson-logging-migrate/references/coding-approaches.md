...
3. **Gates, not grep alone** — `632→0 {}` while Java does not compile is incomplete; run
   [completion-gates.md](completion-gates.md) in full.
...
- [ ] API/DTO/exception text unchanged when user chose **structure at logging boundary** — see
  [pattern-recipes.md](pattern-recipes.md)
- [ ] Conditional `message` / `msg` builders: log uses `.setMessage(sameVariable)`, not a different summary string
