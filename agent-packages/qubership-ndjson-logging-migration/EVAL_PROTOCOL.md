...
The **published skill** has no bundled scripts — completion gates are manual
([completion-gates.md](.apm/skills/qubership-ndjson-logging-migration/references/completion-gates.md)). Eval-only
helpers live under `../qubership-ndjson-logging-migration-workspace/scripts/`.
...
Baseline should **fail** objective checks on dbaas (hundreds of `{}`, no operator migration) even when process
expectations pass. With-skill should pass gates or document explicit `blocked` credentials.
