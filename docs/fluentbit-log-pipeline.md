...
| 26  | filters/filter-validate.conf                  | FILTER modify (Condition Key_value_matches log ^\s*\{...\}\s*$; Set parse_format json; Rename msg short_message) | Match pods*                               | Marks a JSON record with `parse_format: json` and lifts its `msg` or `message` payload into `short_message`.
| 27  | filters/filter-validate.conf                  | FILTER modify (Rename parsed_level level)                                             | Match pods*                               | Restores the application `level` from `parsed_level` for every parsed format. `Rename` keeps an authoritative `level` when the record already carries one.
| 28  | filters/filter-validate.conf                  | FILTER modify (Condition Key_value_matches parse_status success; Copy log short_message)       | Match_regex (pods\|klog).*                | If `parse_status: success`, copies the `log` field to the `short_message` (if `short_message` doesn't exist yet) and renames the `log` field to the `original_log`                                                                      |
| 29  | filters/filter-generic.conf                   | FILTER modify (Copy log _parser_input); FILTER parser (Preserve_Key Off)              | Match pods*                               | Runs generic parsers against `_parser_input`. The first successful parser removes the field, so the remaining parsers skip the record.                                                                                       |
| 30  | filters/filter-post-generic.conf              | FILTER modify (Condition Key_does_not_exist _parser_input)                            | Match pods*                               | Marks a record as successfully parsed when a generic parser removed `_parser_input`.                                                                                                                                         |
| 31  | filters/filter-post-generic                   | FILTER parser (Parser level_parser_common_keep)                                       | Match pods*                               | Tries to extract a log's severity level by defined regex in the mentioned parser                                                                                                                                             |
| 32  | filters/filter-post-generic.conf              | FILTER modify (Copy log short_message)                                                | Match_regex (pods\|klog).*                | Copies the `log` field to the `short_message` if `short_message` doesn't exist yet                                                                                                                                           |
| 33  | filters/filter-post-generic.conf              | FILTER lua (Call kv_parse)                                                            | Match pods*                               | Parses [key=value] labels in `log`, only if ``parse_status: success`` and the `log` field exists and is not empty                                                                                                                       |
| 34  | filters/filter-post-generic.conf              | FILTER modify (Condition Key_value_matches log <regex_here>; Set nc_audit_label true) | Match_regex pods.\*grafana.\*             | Marks messages from grafana containing specific text mentioned in Condition with `nc_audit_label: true`                                                                                                                      |
| 35  | filters/filter-post-generic.conf              | FILTER modify (Condition Key_value_matches log <regex_here>; Set nc_audit_label true) | Match_regex pods.\*mongo.\*               | Marks messages from mongo containing specific text mentioned in Condition with `nc_audit_label: true`                                                                                                                        |
| 36  | filters/filter-post-generic.conf              | FILTER modify (Condition Key_value_matches log <regex_here>; Set nc_audit_label true) | Match pods*                               | Marks messages containing specific text mentioned in Condition with `nc_audit_label: true`                                                                                                                                   |
| 37  | filters/filter-post-generic.conf              | FILTER modify (Condition Key_value_matches parsed true; Hard_rename log original_log) | Match_regex (pods\|klog).*                | If `parsed: true`, renames the `log` field to `original_log` and removes the auxiliary fields used to determine whether the message was parsed                                                                               |
| 38  | filters/filter-post-generic.conf              | FILTER record_modifier (Record hostname ${HOSTNAME})                                  | Match *                                   | Adds the `hostname` field, sourced from a FluentBit pod's environment variable, required for the GELF format                                                                                                                 |
| 39  | filters/filter-post-generic.conf              | FILTER record_modifier (Record nodename ${NODE_NAME})                                 | Match *                                   | Adds the `nodename` field, sourced from a FluentBit pod's environment variable, required for the GELF format                                                                                                                 |
| 40  | filters/filter-nonsupported-levels.conf       | FILTER lua (Script /fluent-bit/etc/update_level_syslog.lua)                           | Match *                                   | Converts the value of the `level` field to FluentBit supported severity levels                                                                                                                                               |
| 41  | filters/filter-nonsupported-levels.conf       | FILTER modify (Rename parsed_source_level source_level)                               | Match *                                   | Restores the application `source_level` only when level normalization did not create the canonical field.                                                                                                                    |
...
| 45  | outputs/output-http.conf                      | OUTPUT http                                                                           | Match *                                   | Sends data in json_lines format to HTTP storage backend (VictoriaLogs)                                                                                                                                                       |
<!-- textlint-enable -->
