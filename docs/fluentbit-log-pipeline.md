...
| 17  | filters/filter-validate.conf                  | FILTER lua (Call kv_parse)                                                            | Match pods*                                  | Parses [key=value] labels in `log` only when `__qubership_candidate` exists                                                                                                                                                  |
...
| 24  | filters/filter-post-generic.conf              | FILTER parser (Parser level_parser_common_keep)                                       | Match pods*                                  | Tries to extract a log's severity level with the common level parser                                                                                                                                                         |
| 25  | filters/filter-post-generic.conf              | FILTER modify (Set parse_format logfmt; Rename message fields)                        | Match pods*                                  | Classifies successfully parsed logfmt records and normalizes their message and time fields                                                                                                                                   |
| 26  | filters/filter-post-generic.conf              | FILTER modify (Set parse_format postgres)                                             | Match_regex pods.\*(postgres\|patroni\|pg).* | Classifies successfully parsed PostgreSQL records                                                                                                                                                                            |
| 27  | filters/filter-post-generic.conf              | FILTER modify (Set parse_format consul)                                               | Match_regex pods.\*consul.\*                 | Classifies successfully parsed Consul records                                                                                                                                                                                |
| 28  | filters/filter-post-generic.conf              | FILTER lua (Call kv_parse)                                                            | Match pods*                                  | Parses [key=value] labels in `log` only when `__qubership_candidate` exists                                                                                                                                                  |
| 29  | filters/filter-post-generic.conf              | FILTER modify (Condition Key_value_matches log <regex_here>; Set nc_audit_label true) | Match_regex pods.\*grafana.\*                | Marks messages from Grafana containing specific text with `nc_audit_label: true`                                                                                                                                             |
| 30  | filters/filter-post-generic.conf              | FILTER modify (Condition Key_value_matches log <regex_here>; Set nc_audit_label true) | Match_regex pods.\*mongo.\*                  | Marks messages from MongoDB containing specific text with `nc_audit_label: true`                                                                                                                                             |
| 31  | filters/filter-post-generic.conf              | FILTER modify (Condition Key_value_matches log <regex_here>; Set nc_audit_label true) | Match pods*                                  | Marks messages containing audit-related text with `nc_audit_label: true`                                                                                                                                                     |
| 32  | filters/filter-post-generic.conf              | FILTER modify (Set parse_format logfmt; Rename message fields)                        | Match pods*                                  | Normalizes message and time fields for remaining logfmt candidates that contain `msg`                                                                                                                                        |
| 33  | filters/filter-post-generic.conf              | FILTER modify (Set parse_format qubership)                                            | Match_regex (pods\|klog).*                   | Classifies records that contain `__qubership_candidate` as Qubership logs                                                                                                                                                    |
| 34  | filters/filter-post-generic.conf              | FILTER modify (Set short_message `<Empty message>`)                                   | Match_regex (pods\|klog).*                   | Adds a placeholder when a Qubership record has no `short_message`                                                                                                                                                            |
| 35  | filters/filter-post-generic.conf              | FILTER modify (Copy log short_message)                                                | Match_regex (pods\|klog).*                   | Uses the raw `log` as a fallback when no parser or placeholder produced `short_message`                                                                                                                                      |
| 36  | filters/filter-post-generic.conf              | FILTER record_modifier (Remove_key temporary and raw-log fields)                      | Match_regex (pods\|klog).*                   | Removes `orig_field_count`, `log`, `original_log`, `logfmt_candidate`, and `__qubership_candidate`                                                                                                                           |
| 37  | filters/filter-post-generic.conf              | FILTER record_modifier (Record hostname ${HOSTNAME})                                  | Match *                                      | Adds the `hostname` field, sourced from a FluentBit pod's environment variable, required for the GELF format                                                                                                                 |
| 38  | filters/filter-post-generic.conf              | FILTER record_modifier (Record nodename ${NODE_NAME})                                 | Match *                                      | Adds the `nodename` field, sourced from a FluentBit pod's environment variable, required for the GELF format                                                                                                                 |
| 39  | filters/filter-nonsupported-levels.conf       | FILTER lua (Script /fluent-bit/etc/update_level_syslog.lua)                           | Match *                                      | Converts the value of the `level` field to FluentBit supported severity levels                                                                                                                                               |
...
| 43  | outputs/output-http.cond                      | OUTPUT http                                                                           | Match *                                      | Sends data in json_lines format to HTTP storage backend (VictoriaLogs)                                                                                                                                                       |
<!-- textlint-enable -->

`__qubership_candidate` is reserved for internal pipeline use. Application log payloads must not define this field.
The pipeline removes it before sending a record to an output.
