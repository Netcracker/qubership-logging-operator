...
        FPC["filter-enrich-fields.conf<br/>first_count_fields detects parsed-field collisions<br/>Lift regular fields from log_parsed<br/>Lift collisions from parsed_fields with parse_ prefix"] --> FRT
...
| 10  | filters/filter-enrich-fields.conf             | FILTER lua (Call first_count_fields)                                                  | Match pods*                               | Counts original fields. Keeps existing root metadata authoritative by moving fields from `log_parsed` that have the same names under `parsed_fields`                                                                         |
| 11  | filters/filter-enrich-fields.conf             | FILTER nest (Operation lift; Remove_prefix log_parsed)                                | Match pods*                               | Moves non-conflicting parsed fields from `log_parsed` to the root level                                                                                                                                                      |
| 12  | filters/filter-enrich-fields.conf             | FILTER nest (Operation lift; Nested_under parsed_fields; Add_prefix parse_)           | Match pods*                               | Moves conflicting parsed fields from `parsed_fields` to the root level with a `parse_` prefix. Repeated collisions receive additional prefixes, for example `parse_parse_namespace`                                          |
| 13  | filters/filter-rewrite-tag.conf               | FILTER rewrite_tag (Rule $pod  ^kube-.* klog.$TAG  false)                             | Match pods*                               | Rewrites tag to klog.$TAG without preserving the original record. Sends the record to the pipeline beginning with the new tag                                                                                                |
| 14  | filters/filter-rewrite-tag.conf               | FILTER parser (Parser klog_entry)                                                     | Match klog*                               | Tries to parse with klog_entry parser. The format is described in [Kubernetes system logs](https://kubernetes.io/docs/concepts/cluster-administration/system-logs/)                                                          |
| 15  | filters/filter-rewrite-tag.conf               | FILTER parser (Parser klog_trace_entry)                                               | Match klog*                               | Tries to parse with klog_trace_entry parser                                                                                                                                                                                  |
| 16  | filters/filter-events-reader.conf             | FILTER modify                                                                         | Match_regex pods.\*events-reader.\*       | Renames involvedObjectNamespace field to namespace                                                                                                                                                                           |
| 17  | filters/filter-validate.conf                  | FILTER lua (Call second_count_fields)                                                 | Match_regex (pods\|klog).*                | Counts fields after parsing by parser suggested in annotations (or json by default). Sets parsed: true if new fields count > original fields count, otherwise parsed: false. Stores the current count in `parse_field_count` |
| 18  | filters/filter-validate.conf                  | FILTER lua (Call kv_parse)                                                            | Match pods*                               | Parses [key=value] labels in `log`, only if parsed: true                                                                                                                                                                     |
...
| 23  | filters/filter-generic.conf                   | FILTER parser (multiple parser filters)                                               | Match pods*                               | Contains the generic filter chain that uses common parsers defined in parsers.conf. Records are processed only if the `log` field exists and is not empty                                                                    |
...
| 25  | filters/filter-post-generic                   | FILTER parser (Parser level_parser_common_keep)                                       | Match pods*                               | Tries to extract a log's severity level by defined regex in the mentioned parser                                                                                                                                             |
| 26  | filters/filter-post-generic.conf              | FILTER modify (Copy log short_message)                                                | Match_regex (pods\|klog).*                | Copies the `log` field to the `short_message` if `short_message` doesn't exist yet                                                                                                                                           |
| 27  | filters/filter-post-generic.conf              | FILTER lua (Call kv_parse)                                                            | Match pods*                               | Parses [key=value] labels in `log`, only if `parsed: true` and the `log` field exists and is not empty                                                                                                                       |
| 28  | filters/filter-post-generic.conf              | FILTER modify (Condition Key_value_matches log <regex_here>; Set nc_audit_label true) | Match_regex pods.\*grafana.\*             | Marks messages from grafana containing specific text mentioned in Condition with `nc_audit_label: true`                                                                                                                      |
| 29  | filters/filter-post-generic.conf              | FILTER modify (Condition Key_value_matches log <regex_here>; Set nc_audit_label true) | Match_regex pods.\*mongo.\*               | Marks messages from mongo containing specific text mentioned in Condition with `nc_audit_label: true`                                                                                                                        |
| 30  | filters/filter-post-generic.conf              | FILTER modify (Condition Key_value_matches log <regex_here>; Set nc_audit_label true) | Match pods*                               | Marks messages containing specific text mentioned in Condition with `nc_audit_label: true`                                                                                                                                   |
| 31  | filters/filter-post-generic.conf              | FILTER modify (Condition Key_value_matches parsed true; Hard_rename log original_log) | Match_regex (pods\|klog).*                | If `parsed: true`, renames the `log` field to `original_log` and removes the auxiliary fields used to determine whether the message was parsed                                                                               |
| 32  | filters/filter-post-generic.conf              | FILTER record_modifier (Record hostname ${HOSTNAME})                                  | Match *                                   | Adds the `hostname` field, sourced from a FluentBit pod's environment variable, required for the GELF format                                                                                                                 |
| 33  | filters/filter-post-generic.conf              | FILTER record_modifier (Record nodename ${NODE_NAME})                                 | Match *                                   | Adds the `nodename` field, sourced from a FluentBit pod's environment variable, required for the GELF format                                                                                                                 |
| 34  | filters/filter-nonsupported-levels.conf       | FILTER lua (Script /fluent-bit/etc/update_level_syslog.lua)                           | Match *                                   | Converts the value of the `level` field to FluentBit supported severity levels                                                                                                                                               |
| 35  | filters/filter-unparsed-log-counter.conf      | FILTER log_to_metrics (Regex parsed ^false$)                                          | Match_regex (pods\|klog).*                | Generates the prometheus metric `fluentbit_parse_error_total`                                                                                                                                                                |
| 36  | outputs/output-graylog.conf                   | OUTPUT gelf                                                                           | Match_regex (audit\|system\|pods\|klog).* | Sends data in GELF format to graylog host defined in the output config                                                                                                                                                       |
| 37  | outputs/output-prometheus-log-to-metrics.conf | OUTPUT prometheus_exporter                                                            | Match parse_error_metrics                 | Exposes the metric `fluentbit_parse_error_total` to the `2021` port                                                                                                                                                          |
| 38  | outputs/output-http.conf                      | OUTPUT http                                                                           | Match *                                   | Sends data in json_lines format to HTTP storage backend (VictoriaLogs)                                                                                                                                                       |
<!-- textlint-enable -->
