...
appear in the record, the FluentBit pipeline adds a `parse_status` field with the value `success`. If the message does not match
any defined parser, it will be marked with `parse_status: failed`.
...
| 16  | filters/filter-validate.conf                  | FILTER lua (Call second_count_fields)                                                 | Match_regex (pods\|klog).*                | Counts fields after parsing by parser suggested in annotations (or json by default). Sets parse_status: success if new fields count > original fields count, otherwise parse_status: failed                                             |
| 17  | filters/filter-validate.conf                  | FILTER lua (Call kv_parse)                                                            | Match pods*                               | Parses [key=value] labels in `log`, only if parse_status: true                                                                                                                                                                |
...
| 21  | filters/filter-validate.conf                  | FILTER modify (Condition Key_value_matches parse_status success; Copy log short_message)       | Match_regex (pods\|klog).*                | If parse_status: success, copies the `log` field to the `short_message` (if `short_message` doesn't exist yet) and renames the `log` field to the `original_log`                                                                 |
