...
| 33  | filters/filter-nonsupported-levels.conf       | FILTER lua (Script /fluent-bit/etc/update_level_syslog.lua)                           | Match regex: `^(?!out_).*` | Converts the value of the `level` field to FluentBit supported severity levels                                                                                                                                               |
...
13) source_level - Preserves an existing value; otherwise, stores `level` after parsing and before normalization.
    If the incoming payload already contains `source_level`, its value is preserved even if it differs from `level`.
    If both fields are absent, `source_level` is set to `""`, including for audit and system logs.
