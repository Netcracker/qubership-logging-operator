...
| `storageType`                     | string                                                                                                                            | no        | `memory`                                                                                        | The buffering mechanism for the input plugins. Possible values: `memory`, `filesystem`. The `filesystem` type buffers chunks on the node disk and produces a lot of writes to the storage. |
| `db.enabled`                      | boolean                                                                                                                           | no        | `true`                                                                                          | Enables the SQLite database where the input plugins keep the position of the read files. Disabling it removes all the database writes to the storage, but FluentBit loses the read offsets on the restart. |
