...
| `output.http.uri`                 | string                                                                                                                            | no        | `/insert/jsonline?_stream_fields=namespace,container&_msg_field=short_message&_time_field=time` | HTTP URI for the target web server. See [VictoriaLogs timestamp handling](#victorialogs-timestamp-handling)                                                                               |
...
| `output.http.extraParams`         | string                                                                                                                            | no        | See example below                                                                               | Additional configuration parameters for http output. `json_date_key` is reserved when the operator-managed default URI is used; custom URIs retain user-defined timestamp settings. See docs: [http output: configuration parameters](https://docs.fluentbit.io/manual/pipeline/outputs/http#configuration-parameters)  |
