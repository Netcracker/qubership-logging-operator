...
| `db.enabled`                      | boolean                                                                                                                           | no        | `true`                                                                                          | Enables the SQLite database where the input plugins keep the position of the read files. Disabling it removes the database writes, but FluentBit loses the read offsets on the restart, so every input starts from the position set by its own `Read_from_Head` or `Read_from_Tail` option: the container log inputs re-read the existing records, and the system and audit log inputs skip the records written while FluentBit was down.  |
